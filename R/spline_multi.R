...
#' spline_multi(phytopknar_ret_ord, ab_treshold = 0.75, obs_year = 2, s_param = 0.35, t_scale = 52, control = 0)
#' }
#' @export
#'
spline_multi <-
  function(x, ab_treshold = 0.75, obs_year = 2, s_param = 0.35, t_scale = 52, control = 0, past = TRUE) {
    df_emp2 <- data.frame()
    for (j in 8:ncol(x)) {
      S.NAME <- names(x[j])
      df1.1 <- x %>% select(1:7, j)

      df2.1 <- spline_uni(df1.1, ab_treshold = ab_treshold, obs_year = obs_year, s_param = s_param, t_scale = t_scale, S.NAME = S.NAME, control = control)
